...
gui.sources.append(
    ArchiveSource(
        gui, {"index": "/build/lat/CMSSW_2_2_10/src/VisMonitoring/DQMServer/foo"}
    )
)
...
for i in xrange(1, 2):  # 20
    ...
    print(
        ...
        gui.sources[0]._data(
            ...
            "0",
            session["dqm.dataset"],
            "EcalBarrel/EventInfo/reportSummaryMap",
        ),
    )
    print(
        i,
        gui.sources[0]._data(
            "scalar", "0", session["dqm.dataset"], "EcalBarrel/EventInfo/reportSummary"
        ),
    )
