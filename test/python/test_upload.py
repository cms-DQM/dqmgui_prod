...
class UploadTest(helper.CPWebCase):
    def setup_server():
        class Root:
            ...
            def put(self, size, xtra, file):
                ...
                return "thanks len %d orig %d file '%s'" % (
                    int(size),
                    filelen,
                    file.filename,
                )
        ...
        cherrypy.tree.mount(
            Root(), "/big", config={"/": {"request.body_io_size": bufsize}}
        )
    ...
    def _run_test(self, kind, kbytes, with_length):
        ...
        print(
            "\nkind: %s,with_length: %s\t time(client=%.3f server=%s)"
            % (kind, str(with_length), time.time() - start, xtiming),
        )

    def test_small_smart(self):
        self._run_test("small", 10, True)

    def test_small_dumb(self):
        self._run_test("small", 10, False)

    def test_big_smart(self):
        self._run_test("big", 150 * 1024, True)

    def test_big_dumb(self):
        self._run_test("big", 150 * 1024, False)
