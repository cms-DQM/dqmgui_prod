...
class SeleniumTest(BaseIntegrationTest):
    ...
    def tearDown(self):
        ...
        if errors_encountered:
            self.fail(
                "JavaScript errors encountered. See test execution output for details."
            )
    ...
    def waitAndFindElement(
        self, selector, by=By.XPATH, presence_function=EC.visibility_of_element_located
    ):
        """
        ...
        try:
            ...
            self.fail(
                'Element was not found by "%s" with selector "%s"' % (by, selector)
            )
    ...
    def test_workspaces_shift(self):
        ...
        self.waitAndFindElement(
            "//div[@id = 'canvas']//a[contains(text(), '%s')]" % "CSC"
        ).click()
    ...
    def test_openHistogram(self):
        ...
        histo = self.waitAndFindElement(
            selector="//h3[contains(text(), '%s')]/following-sibling::div/img"
            % "hALCTMatch"
        )
    ...
    def test_linkMeButton(self):
        ...
        link_me_actual = self.waitAndFindElement(
            "//*[@class = 'ext-mb-input']"
        ).get_attribute("value")
    ...
    def test_rootFileLink(self):
        ...
        root_file_href = self.waitAndFindElementContains("ROOT File").get_attribute(
            "href"
        )
        self.assertTrue(
            root_file_href.endswith(".root"),
            "Root file link must point to a ROOT file.",
        )
    ...
    def test_searchWorkspace(self):
        ...
        time.sleep(1)
        self.assertGreater(
            len(datasets.find_elements_by_xpath(".//*")),
            0,
            "Search must provide results when for CMS",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip-upload", action="store_const", const=True, default=False
    )
    parser.add_argument(
        "--show-browser", action="store_const", const=True, default=False
    )
