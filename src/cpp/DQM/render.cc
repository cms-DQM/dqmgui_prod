...
static int colors[] = {kGray,    kAzure,       kOrange + 7,
                       kRed + 1, kMagenta + 2, kGreen - 3};
...
static bool parseDouble(const char *&p, const char *name, size_t len,
                        double &value) {
  if (!strncmp(p, name, len)) {
...
static bool parseReference(const char *&p, const char *name, size_t len,
                           VisDQMReference &value) {
  if (!strncmp(p, name, len)) {
    p += len;
...
static bool parseStripChart(const char *&p, const char *name, size_t len,
                            VisDQMStripTrend &value) {
  if (!strncmp(p, name, len)) {
    p += len;
    if (!strncmp(p, "object", 6)) {
...
static bool parseAxisType(const char *&p, const char *name, size_t len,
                          std::string &value) {
  if (!strncmp(p, name, len)) {
    p += len;
    if (!strncmp(p, "def", 3) || !strncmp(p, "lin", 3) ||
        !strncmp(p, "log", 3)) {
      value.clear();
...
static bool parseOption(const char *&p, const char *name, size_t len,
                        std::string &value) {
...
static bool parseImageSpec(VisDQMImgInfo &i, const std::string &spec,
                           const char *&error) {
  error = 0;
  i.imgspec = spec;
  i.width = -1;
...
TObject *makeStripChart(VisDQMImgInfo &info, uint32_t flags,
                        const std::string &title, std::vector<double> &axisvals,
                        std::vector<VisDQMIndex::Summary> &attrs,
                        std::vector<std::string> &labels, VisDQMObject *objs,
                        uint32_t numobjs) {
  std::string titlepfx;
  const char *axispfx[] = {"X ", "Y ", "Z ", ""};
  size_t axis =
      (info.trend >= DQM_TREND_X_MEAN && info.trend <= DQM_TREND_X_NUM_BINS ? 0
       : info.trend >= DQM_TREND_Y_MEAN && info.trend <= DQM_TREND_Y_NUM_BINS
           ? 1
       : info.trend >= DQM_TREND_Z_MEAN && info.trend <= DQM_TREND_Z_NUM_BINS
           ? 2
           : 3);
  ...
  case DQM_TREND_NUM_BINS:
    ...
    for (size_t i = nextra; i != nattrs; ++i)
      vals[nattrs - i - 1] =
          attrs[i].nbins[0] * attrs[i].nbins[1] * attrs[i].nbins[2];
    if (extra)
      vals[nattrs - 1] =
          extra->GetNbinsX() * extra->GetNbinsY() * extra->GetNbinsZ();
    break;
  ...
  TH1D *bins =
      new TH1D("Trend", fulltitle.c_str(), realsize, &axisvals[nattrsmin]);
  ...
  for (size_t i = 0, e = vals.size(); i != e; ++i) {
    ...
    // first).
    size_t labelIndex =
        (info.trend == DQM_TREND_LS_VALUE) ? nattrsmin + i : (nattrs - i - 1);
    if (i % skip == 0)
      if (i < realsize)
  ...
  if (yerr.empty() && yhi.empty())
    ...
    graph = new TGraphErrors(realsize, &axisvals[nattrsmin], &vals[nattrsmin],
                             0, &yerr[nattrsmin]);
  else if (!yhi.empty())
    graph =
        new TGraphAsymmErrors(realsize, &axisvals[nattrsmin], &vals[nattrsmin],
...
public:
  VisDQMImageServer(const ServerInfo &info)
    ...
    logme() << "INFO: DQM image server started in path " << info_.statedir
            << std::endl;

    for (BlackList::const_iterator i = info.blacklist.begin(),
                                   e = info.blacklist.end();
         i != e; ++i)
      logme() << "INFO: black-listed '" << i->first << "' since "
              << i->second.format(true, "%Y-%m-%d %H:%M:%S.")
              << i->second.nanoformat(3, 3) << " (unix time " << i->second
              << ")\n";
...
protected:
  ...
  virtual bool shouldStop(void) { return info_.stopfile.exists(); }
  ...
  // have to wait for.
  virtual void releaseFromWait(Bucket * /* msg */, WaitObject & /* w */,
                               Object * /* o */) {
    ASSERT(false);
  }
  ...
  virtual bool onMessage(Bucket *msg, Peer *p, unsigned char *data,
    ...
    if (type == DQM_MSG_DUMP_PROFILE) {
      if (profdump_) {
        char fname[128];
        sprintf(fname, "|gzip -9c>igprof.visDQMRender.%d.%u.gz", (int)getpid(),
                (int)time(0));
        profdump_(fname);
      }
      return true;
      ...
      if (debug_)
        logme() << "DEBUG: received message 'GET IMAGE DATA' from peer "
                << p->peeraddr << std::endl;
      ...
      if (len != gotlen) {
        logme() << "ERROR: corrupt 'GET IMAGE DATA' message of length " << len
                ...
                << (nwords * sizeof(uint32_t)) << " + " << namelen << " + "
                << speclen << " + " << datalen << " + " << qlen << " = "
                << gotlen << std::endl;
        return false;
      }
      ...
      if (type == DQM_MSG_GET_IMAGE_DATA &&
          !parseImageSpec(info, spec, error)) {
        logme() << "ERROR: invalid image specification '" << spec
                << "' for object '" << name << "': " << error << std::endl;
        return false;
      }
      ...
      logme() << "INFO: rendered '" << objs[0].name << "' version "
              << objs[0].version << " as '" << spec
              << (blacklisted ? "', black-listed" : "'") << ", " << numobjs
              << " objects, main object had" << (hadref ? "" : " no")
              << " reference, in " << ((end - start).ns() * 1e-3) << " us\n";
...
private:
  void initPlugins(void) {
    ...
    if (!info_.dynobj.empty()) {
      ...
      for (size_t i = 0, e = plugins_.size(); i != e; ++i) {
        if (info_.debug)
          logme() << "initialising plugin #" << i << ": "
                  << typeid(*plugins_[i]).name() << "\n";
  ...
  // ----------------------------------------------------------------------
  bool readRequest(VisDQMImgInfo &info, std::string &odata,
                   std::vector<VisDQMObject> &objs, uint32_t &numobjs) {
    std::vector<VisDQMIndex::Summary> attrs;
    ...
    if (info.trend != DQM_TREND_OBJECT) {
      ...
      if (!(objs[0].flags & DQM_PROP_TYPE_DATABLOB)) {
        logme() << "ERROR: invalid data type for a trend plot, expected"
                << " a data blob, got flags 0x" << std::hex << objs[0].flags
                << std::dec << " and payload of size " << odata.size()
                << " for " << numobjs << " objects\n";
        return false;
      }
      ...
      if (odata.size() - pos < sizeof(lenbits)) {
        logme() << "ERROR: 'GET IMAGE DATA' missing trend data length"
                ...
                << (odata.size() - pos) << " bytes left of " << odata.size()
                << " for trend attributes\n";
        return false;
      }
    ...
    if ((objs[0].flags & DQM_PROP_TYPE_MASK) <= DQM_PROP_TYPE_SCALAR) {
      ...
      if (pos != odata.size()) {
        logme() << "ERROR: 'GET IMAGE DATA' request with excess"
                << " object data, expected " << pos << " bytes but found "
                << odata.size() << std::endl;
        return false;
      }
    }
    ...
    if (info.trend != DQM_TREND_OBJECT) {
      objs[0].object =
          makeStripChart(info, objs[0].flags, objs[0].objname, axisvals, attrs,
                         binlabels, &objs[0], numobjs);
      if (numobjs > 1) {
  ...
  // ----------------------------------------------------------------------
  bool readJsonRequest(std::string &odata, std::vector<VisDQMObject> &objs,
                       uint32_t &numobjs) {
    std::vector<VisDQMIndex::Summary> attrs;
    std::vector<std::string> binlabels;
    std::vector<double> axisvals;
  ...
  void getJson(VisDQMObject *objs, size_t /*numobjs*/, DataBlob &jsondata) {
    ...
    if (const TH1 *const h = dynamic_cast<const TH1 *const>(ob)) {
      ...
      if (const TH2 *const h2 = dynamic_cast<const TH2 *const>(ob)) {
        if (const TProfile2D *const h2d =
                dynamic_cast<const TProfile2D *const>(ob)) {
          json += StringFormat(jsonTemplate)
                      .arg(h2d->Class_Name())
                      .arg(optionalTextValueToJson<const char *const>(
                          "title", h2d->GetTitle()))
                      ...
                      .arg(h2d->GetMaximum())
                      .arg(binsToArray(h2d))
                      .arg(optionalNumericValueToJson<Double_t>("zMin",
                                                                h2d->GetZmin()))
                      .arg(optionalNumericValueToJson<Double_t>("zMax",
                                                                h2d->GetZmax()))
        ...
        if (const TProfile *const tprof =
                dynamic_cast<const TProfile *const>(ob)) {
          json += StringFormat(jsonTemplate)
                      .arg(tprof->Class_Name())
                      .arg(optionalTextValueToJson<const char *const>(
                          "title", tprof->GetTitle()))
                      ...
                      .arg(optionalNumericValueToJson<Double_t>(
                          "yMax", tprof->GetYmax()))
                      .arg(optionalNumericValueToJson<Double_t>(
                          "yMin", tprof->GetYmin()))
  ...
  void drawReferenceStatBox(VisDQMImgInfo &i, size_t order, TH1 *ref, int color,
    ...
    // causing us problems.
    TPaveStats *currentStat = new TPaveStats(
        0.78, 0.835 - (order + 1) * 0.16, 0.98, 0.835 - order * 0.16, "brNDC");
    if (currentStat) {
  ...
  double calculateScalingFactor(VisDQMObject *objs, size_t numobjs) {
    ...
    return data_area > 0
               ? (total_stack_area > 0.0 ? data_area / total_stack_area : 1.0)
               : 1.0;
  }

  // ----------------------------------------------------------------------
  // Render arbitraty text message into the TCanvas.
  void doRenderMsg(std::string histogram_name, const char *msg,
                   const Color_t &c, std::vector<TObject *> &nukem) {
    std::string label;
  ...
  void computeMinAndMaxForEfficiency(const TH1 *h, double ratio_min,
                                     double ratio_max, double &minimum,
                                     double &maximum) {
  ...
  void doRenderOverlayAndRatio(TCanvas &c, VisDQMImgInfo &i, VisDQMObject *objs,
                               size_t numobjs, const VisDQMRenderInfo &ri,
                               std::vector<TObject *> &nukem) {
    TObject *ob = objs[0].object;
    // Do normal rendering for everything but TH1*
    if (dynamic_cast<TH2 *>(ob) || dynamic_cast<TProfile *>(ob) ||
    ...
    if ((h = dynamic_cast<TH1 *>(ob)) && (h->GetEntries() != 0)) {
      ...
      for (size_t n = 0; n < numobjs; ++n) {
        ...
        if (TH1 *den = dynamic_cast<TH1 *>(objs[n].object)) {
          ...
          if (sum) {
            ...
            computeMinAndMaxForEfficiency(num, RATIO_MIN, RATIO_MAX, ratio_min,
                                          ratio_max);
            ...
            ratio_shadow_error_range->SetFillColor(
                TColor::GetColorTransparent(colors[n % colorIndex], 0.3));
            ratio_shadow_error_range->SetLineColor(
                TColor::GetColorTransparent(colors[n % colorIndex], 0.3));
            ratio_shadow_error_range->SetMarkerColor(
                TColor::GetColorTransparent(colors[n % colorIndex], 0.3));
            ratio_shadow_error_range->SetMarkerSize(0);
            for (int bin = 1; bin < ratio_shadow_error_range->GetNbinsX();
                 ++bin)
      ...
      for (size_t histo = histograms_to_be_drawn; histo < nukem.size();
           ++histo) {
  ...
  void doRenderStacked(TCanvas &c, VisDQMImgInfo &i, VisDQMObject *objs,
                       size_t numobjs, const VisDQMRenderInfo & /* ri */,
                       std::vector<TObject *> &nukem) {
    int colorIndex = sizeof(colors) / sizeof(int);
    ...
    if (TH1 *dataHistogram = dynamic_cast<TH1 *>(objs[0].object)) {
      ...
      // Calculate what each the histograms that are to be stacked should be
      // scaled by to match the data histogram
    ...
    if (i.showstats)
      for (size_t n = 1; n < numobjs; ++n)
        drawReferenceStatBox(i, n, dynamic_cast<TH1 *>(objs[n].object),
                             colors[n % colorIndex], objs[n].name, nukem);
  }

  // ----------------------------------------------------------------------
  // Ordinary rendering of a ROOT object.
  void doRenderOrdinary(TCanvas &c, VisDQMImgInfo &i, VisDQMObject *objs,
                        size_t numobjs, const VisDQMRenderInfo &ri,
                        std::vector<TObject *> &nukem) {
    ...
    for (size_t n = 0; n < numobjs; ++n) {
      ...
      // same canvas. Bail out w/o complaints.
        ...
        if (n == 1 && (!isnan(i.ktest)) && h && norm && ref1 &&
            ref1->GetSumOfWeights()) {
        ...
        if (norm &&
          ...
          if (ref1->GetSumOfWeights())
            nukem.push_back(
                ref1->DrawNormalized(samePlotOptions.c_str(), norm));
        } else {
  ...
  bool doRender(VisDQMImgInfo &i, VisDQMObject *objs, size_t numobjs,
                DataBlob &imgdata) {
    ...
    if (TH1 *h = dynamic_cast<TH1 *>(ob)) {
      h->SetMinimum();
      h->SetMaximum();
      if (isnan(h->Integral()) || isnan(h->GetRMS()) ||
    ...
    if (!ob) {
      Color_t c = TColor::GetColor(64, 64, 64);
      doRenderMsg(o.name,
                  i.reference != DQM_REF_REFERENCE ? "is not available now"
                                                   : "has no reference",
                  c, nukem);
    }
...
int main(int argc, char **argv) {
  static const int FATAL_OPTS =
      (lat::Signal::FATAL_DEFAULT &
       ~(lat::Signal::FATAL_ON_INT | lat::Signal::FATAL_ON_QUIT |
