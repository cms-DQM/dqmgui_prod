from ROOT import gDirectory, TBufferFile, TClass, kTRUE
...
def tfile_cd(dirname, tfile, debug=False):
    ...
    if debug:
        print(f"Current dir {gDirectory.pwd()}")
    ...
    while tbuffer.Length() != tbuffer.BufferSize():
        ...
        if c:
            ...
            if c.GetStreamerInfos().At(v):
                if debug:
                    print(
                        f"skipping already present streamer info version {v} for {obj.GetName()}"
                    )
                continue
        if debug:
            print(f"Importing streamer info version {v} for {obj.GetName()}")
        obj.BuildCheck()
