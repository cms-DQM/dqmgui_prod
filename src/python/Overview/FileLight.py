...
class FileLightWorkspace(CompWorkspace):
    ...
    class FileLightGroupView(FileLightView):
        ...
        viewdef = {"height": 800, "width": 800, "type": "png", "group": "null"}
    ...
    def __init__(self, gui, rank, category, name, *args):
        ...
        CompWorkspace.__init__(
            ...
            "filelight",
            name,
            [self.FileLightSiteView(self), self.FileLightGroupView(self)],
        )
...
class FileLightSource(object):
        ...
        if len(path) == 2:
            ...
            if what == "site":
                ...
                if "scheme" in attrs:
                    ...
                    attrs["scheme"] = False
                return self.types[attrs["type"]], self.cache.lockedCacheFetch(
                    "FL_NODEPLOT:%s:%s:%sx%s:%s:%s:%s:%s:%s"
                    % (
                        which,
                        attrs["type"],
                        attrs["width"],
            ...
            if what == "group":
                return self.types[attrs["type"]], self.cache.lockedCacheFetch(
                    "FL_GROUPPLOT:%s:%s:%sx%s"
                    % (which, attrs["type"], attrs["width"], attrs["height"]),
                    self._makeGroupPlot,
                    which,
                    attrs,
                )
    ...
    def _makeSitePlot(self, site, attrs):
        ...
        plot = BaobabChart(
            ...
            None,
            {
                ...
                "width": attrs["width"],
                "title": "%s\n%s"
                % (site, time.strftime("%H:%M %d/%m/%Y", time.gmtime())),
                "dpi": 72,
                "unit": "B",
            },
        )
    ...
    def _makeGroupPlot(self, group, attrs):
        sizes = self.cache.lockedCacheFetch(
            "FL_GROUP:%s" % group, self._groupData, group
        )
        ...
        plot = BaobabChart(
            ...
            None,
            {
                ...
                "width": attrs["width"],
                "title": "%s\n%s"
                % (group, time.strftime("%H:%M %d/%m/%Y", time.gmtime())),
                "dpi": 72,
                "unit": "B",
            },
        )
    ...
    def _nodeData(self, node):
        data = self._fetchJSON(
            "http://cmsweb.cern.ch/phedex/datasvc/json/prod/blockreplicas?node=%s"
            % node
        )
    ...
    def _groupData(self, group):
        data = self._fetchJSON(
            "http://cmsweb.cern.ch/phedex/datasvc/json/prod/groupusage?group=%s" % group
        )
